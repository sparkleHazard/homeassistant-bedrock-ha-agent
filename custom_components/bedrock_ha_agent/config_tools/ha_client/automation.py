...
async def reload_automations(hass: "HomeAssistant") -> None:
    ...
    v1.1.13 added a registry-orphan cleanup pass here, but the
    state=unavailable + restored=true fingerprint matches real
    automations that happen to be unavailable at reload time (e.g. their
    backing integration is still booting). We deleted a live automation
    that way; reverted. If HA exposes a proper "orphaned stub" signal
    in the future we can reintroduce the cleanup behind that.
    """
